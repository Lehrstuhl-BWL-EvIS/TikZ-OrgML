...
\def\memoxorconn#1#2#3#4{
    \@ifundefined{pgf@sh@ns@#2_decision}{
        ...
        \node[#1decision, shape=#1decision] (#2_decision) at ($(#2) + (1.6,0)$) {};
    }{
...
%            #6: Kommentar
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\def\memoxorprobabilityconn#1#2#3#4#5#6{
    \@ifundefined{pgf@sh@ns@#2_decision}{
        %~~ Der Node für die Darstellung der Entscheidung wird für die gesamte
        %~~ Entscheidung nur einmal erzeugt. Der Knoten wird im Abstand 1.6 Einheiten
        %~~ zum ausgehenden Prozess erzeugt.
    ...
    \draw[memoline, shorten <= 3pt] (#2_decision.#4) -- (#3.west) node [midway, fill=probabilitygrey, rounded corners, label={below:\sf #6}] {\sf\textcolor{textwhite}{#5}};
}
