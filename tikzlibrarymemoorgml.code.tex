...
\def\memoexception#1#2#3#4#5{
    \node[#1exception, shape=#1exception] (#2) at (#3, #4) [label={below:#5}] {};
}
...
\def\memoconnxorprobability#1#2#3#4#5#6{
    \memoxor{#1}{#2}    
    \draw[memoline, shorten <= 3pt] (#2_decision.#4) -- (#3.west) node [midway, fill=probabilitygrey, rounded corners, label={below:\sffamilyfamily\tiny #6}] {\tiny\sffamily\textcolor{textwhite}{#5}};
}
...
\def\memocomment#1#2#3#4{
    \node[draw] (#1_comment) at (#3,#4) [label={[draw, align=left, dashed]right:\sffamily #2}] {\sffamily c#1}
}
...
\def\memoconstraint#1#2#3#4{
    \node[draw, fill=memoconstraintred, text=white] (#1_constraint) at (#3,#4) [label={[draw, align=left, memoconstraintred, text=black]right:\sffamily #2}] {\sffamily c#1}
}
\makeatother
